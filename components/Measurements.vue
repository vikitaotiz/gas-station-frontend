  ...
  >
    <template v-slot:top>
      ...
      <v-toolbar flat color="white">
        <v-toolbar-title>Measurements</v-toolbar-title>
        <v-spacer></v-spacer>

        <v-text-field
            v-model="search"
            append-icon="mdi-magnify"
            label="Search Measurement Name..."
            single-line
            dense
            hide-details
          ></v-text-field>
          ...
          <v-card>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  <v-col>
                    <v-text-field autofocus v-model="editedItem.name" label="Measurement Name"></v-text-field>
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>
      ...
      <v-simple-table dense v-if="item.users.length > 0">
        ...
        <tbody>
          <tr v-for="(user, index) in item.users" :key="index">
            <td v-if="user.name !== 'Pos Admin'">{{user.name}}</td>
          </tr>
        </tbody>
      </v-simple-table>
    </template>
...
<v-dialog v-model="dialogConfirmDelete" persistent max-width="300">
      <v-card>
        <v-card-title class="headline">Confirm Measurement Deletion?</v-card-title>
        <v-divider></v-divider>
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn color="red" text @click="deleteItem(deletedMeasurement)">Confirm</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
<script>
  export default {
    ...
    computed: {
      ...
      filteredData(){
          return this.measurements.filter((text) => {
    ...
    methods: {
      ...
      confirmDelete(item){
        ...
        this.deletedMeasurement = item
      },

      deleteItem (item) {
        const index = this.measurements.indexOf(item)

        let delete_user = this.$axios.delete(`/measurements/${item.id}`)
           .then(response => {
             this.dialogConfirmDelete = false
             this.$toast.success(`Measurement created successfully!`, {
               duration: 1000
             })
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          this.$axios.patch(`/measurements/${this.editedItem.id}`, { name: this.editedItem.name })
           .then(response => {
             console.log('1234')
              // Object.assign(this.measurements[this.editedIndex], this.editedItem)
           }).catch(error => { console.log(error) })
        } else {
          this.$axios.post(`/measurements`, { name: this.editedItem.name })
           .then(response => {
             this.$toast.success(`Measurement deleted successfully!`, {
               duration: 1000
             })
             this.measurements.push(response.data.data)
           }).catch(error => { console.log(error.data) })
          //this.measurements.push(this.editedItem)
        }
        this.close()
      },
    },
